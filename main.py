...
def gettingEmpDetails():
    ...
    try:
        if  4<=len(name)>0:
            ...
            messagebox.showinfo("Invalid Value", "Name can not be empty and must have more than 4 chracter")
            return
    except:
        messagebox.showinfo("Invalid Value", "Name should not be empty")
        return


    try:
        if (type(float(mobile)) == float or type(int(mobile)) == int):
            pass
        else:
            messagebox.showinfo("Invalid Value", "Mobile no. should be in digits")
            return
    except:
        messagebox.showinfo("Invalid Value", "Mobile no. should be in digits")
        return

    sql = "insert into " + empTable + " values ('" + EmpId + "','" + name + "','" + password + "','" + dept + "','" + doj + "','" + mobile + "')"
    try:
        cur.execute(sql)
        con.commit()
    except:
...
def EmpRegister():
    ...
    # Employee mobile no.
    lb5 = Label(labelFrame, text="Mobile no. : ", bg='#044F67', fg='white')
...
headingLabel = Label(headingFrame2, text="Welcome to SGTB Library", fg='black')
